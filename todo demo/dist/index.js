...
//             ({ name: item.task, value: item.taskId }))
//     }).then(answers => {
...
let todos = [
    ...
    new TodoItem_js_1.TodoItem(5, "send an email", true)
];
...
function promptComplete() {
    ...
    inquirer.prompt({
        type: "checkbox", name: "complete",
        message: "Mark Tasks Complete",
        choices: collection.getTodoItems(showCompleted).map(item => ({ name: item.task, value: item.taskId }))
    }).then(answers => {
